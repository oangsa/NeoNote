...
- Keep editor font metrics centralized in Slint properties. Render every row's visible text through the same full-line `Text` item, regardless of cursor focus. Draw the cursor as a background rectangle behind that text, positioned from the measured cursor-prefix width and sized from the measured cursor-cell width. Do not reconstruct the focused line as prefix/cell/suffix text segments; it can drift from non-focused row placement.
