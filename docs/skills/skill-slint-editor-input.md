...
- Keep editor font metrics centralized in Slint properties. Render each line as an independent fixed-height row. For non-cursor rows, a single text item is fine; for the cursor row, render prefix, cursor cell, and suffix as row-local segments with the cursor character drawn inside the block. Set segmented cursor-row `HorizontalLayout` alignment to `start`; otherwise Slint can center the row contents when the prefix is empty. Size the normal-mode block from the cursor cell text's own `preferred-width`; do not force a guessed monospace width or overlay a block on a separately rendered full line.
