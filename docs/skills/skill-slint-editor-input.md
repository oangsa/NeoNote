...
- Keep editor font metrics centralized in Slint properties. Render each line as an independent fixed-height row; draw text and cursor from the same row-local origin. Position the cursor from the Slint-measured width of the exact cursor-prefix text supplied by Rust. Do not multiply a guessed monospace cell width by column; fallback/proportional font metrics can make the cursor drift horizontally.
