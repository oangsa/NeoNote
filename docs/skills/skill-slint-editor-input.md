...
- Keep editor font metrics centralized in Slint properties. Render each line as an independent fixed-height row, and use the same row-local `HorizontalLayout` path for cursor and non-cursor rows. Non-cursor rows should render as full text plus a zero-width cursor cell; cursor rows should render prefix, cursor cell, and suffix with the cursor character drawn inside the block. Set alignment to `start`. Size the normal-mode block from the cursor cell text's own `preferred-width`; do not force a guessed monospace width or overlay a block on a separately rendered full line.
